...
// ErrRelayRootNotFound is returned when no .provasign/ directory exists at or above the start path.
var ErrRelayRootNotFound = errors.New("no .provasign/ directory found in this path or any parent")

// DiscoverRelayRoot walks upward from start looking for a directory containing `.provasign/`.
// Returns the directory containing `.provasign/` (NOT the .relay path itself).
func DiscoverRelayRoot(start string) (string, error) {
	...
	cur := abs
	for {
		candidate := filepath.Join(cur, ".provasign")
...
// LoadRelayConfig discovers `.provasign/provasign.yaml` upward from start and loads it.
...
func LoadRelayConfig(start string) (*core.RelayConfig, error) {
	...
	cfgPath := filepath.Join(root, ".provasign", "provasign.yaml")
	data, err := os.ReadFile(cfgPath)
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			// .provasign/ exists but no relay.yaml — return defaults.
