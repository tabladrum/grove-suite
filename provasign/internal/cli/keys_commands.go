...
// `provasign keys gen|fingerprint|export|import` — laptop-mode Ed25519 admission
...
// RunKeys dispatches `provasign keys <sub>`.
func RunKeys(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: provasign keys <gen|fingerprint|export|import>")
		return 1
	}
	switch args[0] {
	case "gen":
...
//   - --user: ~/.provasign/keys/
//   - default: <repo>/.provasign/keys/ (caller's cwd by default)
func keysDir(repo string, user bool) (string, error) {
	if user {
		...
		return filepath.Join(home, ".provasign", "keys"), nil
	}
	if repo == "" {
		wd, err := os.Getwd()
		if err != nil {
	...
	return filepath.Join(abs, ".provasign", "keys"), nil
}

func cmdKeysGen(args []string) int {
	fs := flag.NewFlagSet("keys gen", flag.ContinueOnError)
	repo := fs.String("repo", ".", "repo root (default: cwd)")
	user := fs.Bool("user", false, "store key in ~/.provasign/keys/ instead of <repo>/.provasign/keys/")
	...
	fmt.Println("Use `provasign keys export` to copy it to a new machine.")
	return 0
}
...
func cmdKeysFingerprint(args []string) int {
	...
	user := fs.Bool("user", false, "read from ~/.provasign/keys/ instead of <repo>/.provasign/keys/")
	if err := fs.Parse(args); err != nil {
		return 1
	}
	dir, err := keysDir(*repo, *user)
	...
	if _, err := os.Stat(filepath.Join(dir, "signing.ed25519.key")); err != nil {
		fmt.Fprintf(os.Stderr, "no key at %s; run `provasign keys gen` first\n", dir)
		return 1
	}
	s, err := signer.LoadOrCreateLocal(dir)
	if err != nil {
...
func cmdKeysExport(args []string) int {
	...
	if *out == "" {
		fmt.Fprintln(os.Stderr, "usage: provasign keys export --out <bundle.tar> [--repo <dir>|--user]")
		return 1
	}
	dir, err := keysDir(*repo, *user)
	if err != nil {
...
func cmdKeysImport(args []string) int {
	...
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: provasign keys import <bundle.tar> [--repo <dir>|--user] [--force]")
		return 1
	}
	dir, err := keysDir(*repo, *user)
	if err != nil {
