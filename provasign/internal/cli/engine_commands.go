...
import (
	...
	"github.com/provasign/provasign/internal/admission"
	...
	_ "github.com/provasign/provasign/internal/analyzers/eslint"
	_ "github.com/provasign/provasign/internal/analyzers/external"
	_ "github.com/provasign/provasign/internal/analyzers/gitleaks"
	_ "github.com/provasign/provasign/internal/analyzers/govulncheck"
	_ "github.com/provasign/provasign/internal/analyzers/inlinesecrets"
	_ "github.com/provasign/provasign/internal/analyzers/ruff"
	_ "github.com/provasign/provasign/internal/analyzers/semgrep"
	_ "github.com/provasign/provasign/internal/analyzers/sonar"
	"github.com/provasign/provasign/internal/cert"
	"github.com/provasign/provasign/internal/cert/analysis"
	"github.com/provasign/provasign/internal/cert/build"
	...
	"github.com/provasign/provasign/internal/policy/coverage"
	"github.com/provasign/provasign/internal/policy/deps"
	"github.com/provasign/provasign/internal/policy/fileclass"
	"github.com/provasign/provasign/internal/policy/secrets"
	"github.com/provasign/provasign/internal/profiles"
	"github.com/provasign/provasign/internal/runner/gotest"
	"github.com/provasign/provasign/internal/runner/multilang"
	"github.com/provasign/provasign/internal/runner/nodetest"
	"github.com/provasign/provasign/internal/runner/pytest"
	"github.com/provasign/provasign/internal/signer"
	"github.com/provasign/provasign/internal/stacks"
)
...
// cmdInit scaffolds a minimal `.provasign/` in the current directory.
func cmdInit(args []string) int {
	fs := flag.NewFlagSet("init", flag.ContinueOnError)
	dir := fs.String("dir", ".", "directory in which to create .provasign/")
	...
	cfgPath := filepath.Join(relayDir, "provasign.yaml")
	...
	if *stack != "" {
		...
		for _, p := range written {
			fmt.Printf("wrote .provasign/%s\n", p)
		}
		for _, p := range skipped {
			fmt.Printf("skipped (exists) .provasign/%s\n", p)
		}
	} else if *profile != "" {
		...
		const tmpl = `provasign_version: "0.1"
...
`
	...
	// .provasign/intents/ — committed intent YAMLs (Auto-Intent Capture promotion target).
	if err := os.MkdirAll(filepath.Join(relayDir, "intents"), 0o755); err != nil {
		fmt.Fprintln(os.Stderr, "mkdir intents:", err)
		return 1
	}
	// .provasign/.cache/ — gitignored mutable state owned by the daemon (SQLite, intent drafts, indexer caches).
	if err := os.MkdirAll(filepath.Join(relayDir, ".cache", "intents"), 0o755); err != nil {
		fmt.Fprintln(os.Stderr, "mkdir cache:", err)
		return 1
	}
	// Write .provasign/.gitignore so .cache/, transient state, and local keys never get committed.
	gitignorePath := filepath.Join(relayDir, ".gitignore")
	if _, err := os.Stat(gitignorePath); os.IsNotExist(err) {
		const gi = `# Written by ` + "`provasign init`" + `. Mutable state owned by the laptop daemon.
.cache/
keys/
*.local.yaml
`
		...
		fmt.Println("wrote .provasign/.gitignore")
	}
	// Wire provasign tools with detected AI coding agents: append Pre-Flight
...
// cmdCert handles `provasign cert <verify|show|replay> <ref>`.
// `ref` is a commit SHA for show/verify/replay; show/replay also accept a
// certificate ID.
func cmdCert(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: provasign cert <verify|show|replay> [--jsonld] <ref>")
		return 1
	}
	sub := args[0]
	rest := args[1:]
	...
	if len(positional) < 1 {
		fmt.Fprintf(os.Stderr, "usage: provasign cert %s [--jsonld] <ref>\n", sub)
		return 1
	}
	ref := positional[0]
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "discover .provasign/:", err)
		return 1
	}
	store, err := enginestore.Open(filepath.Join(root, ".provasign", "engine.db"))
	if err != nil {
	...
	case "show":
		...
		return 0
	case "verify":
		pubPath := filepath.Join(root, ".provasign", "keys", "signing.ed25519.pub")
...
func BuildEngine(start string) (*engine.Engine, func(), error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("discover .provasign/: %w", err)
	}
	...
	keyDir := filepath.Join(root, ".provasign", "keys")
