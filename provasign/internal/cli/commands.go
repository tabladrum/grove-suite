...
import (
	...
	"github.com/provasign/provasign/internal/api"
	"github.com/provasign/provasign/internal/config"
	"github.com/provasign/provasign/internal/core"
	relayDB "github.com/provasign/provasign/internal/db"
	...
	ghConn "github.com/provasign/provasign/internal/integration/github"
	jiraConn "github.com/provasign/provasign/internal/integration/jira"
	"github.com/provasign/provasign/internal/lifecycle"
	"github.com/provasign/provasign/internal/routing"
...
func cmdServe(args []string) int {
	...
	cfgPath := fs.String("config", "provasign.yaml", "config file path")
...
func openStores() (*config.Config, *relayDB.IntentStore, *relayDB.RepoStore, *relayDB.ProjectStore, *relayDB.IntegrationStore, error) {
	cfg, err := config.Load("provasign.yaml")
	if err != nil {
...
Laptop-mode workflow (single developer, no Postgres required):
  provasign init                          initialize .provasign/ in the current repo
  provasign keys gen                      generate Ed25519 signing key (mode 0600)
  provasign keys fingerprint              print key id + public-key hex
  provasign keys export --out <bundle>    export keys as a tarball for portability
  provasign keys import <bundle>          import keys from a tarball
  ...
  provasign cert verify|show|replay <ref> inspect or re-run a stored certificate
  provasign hook install|uninstall        manage the git pre-push hook
  provasign mcp serve                     run the MCP server over stdio
  provasign mcp install-for <client>      auto-register relay in IDE MCP config
                                      <client> in {claude-code, cursor, continue, windsurf}
  provasign mcp install-for --list        print supported clients
	relay local init|uninstall|status   one-command laptop setup/teardown/status
	provasign tools list|install|uninstall  manage analyzer toolchain
...
Other:
  ...
  provasign outbox <subcommand>           inspect the outbox reconciler
