...
func TestToJSONLD_IncludesContextAndType(t *testing.T) {
	...
	doc := toJSONLD(c)
	if doc["@context"] != "https://provasign.dev/cert/v1" {
...
func TestE2E_CertReplay_ConfigDrift(t *testing.T) {
	...
	// Mutate .provasign/provasign.yaml so the current effective config hash differs.
	// Add a real policy block (comments don't affect the parsed config hash).
	cfgPath := filepath.Join(repo, ".provasign", "provasign.yaml")
	drifted := []byte(`provasign_version: "0.1"
policies:
  size:
    enabled: true
    options:
