...
//
// Combines (in order):
//   1. `provasign tools install --with-sonar`  — fetch jre, sonarlint-ls, plugins,
...
func cmdLocalInit(args []string) int {
	...
	skipTools := fs.Bool("skip-tools", false, "skip `provasign tools install` (use when binaries are already provisioned)")
	...
	fmt.Fprintln(w, "  provasign doctor                  # verify install")
...
func cmdLocalUninstall(args []string) int {
	...
	repo := fs.String("repo", ".", "path to the workspace git repository")
	keepTools := fs.Bool("keep-tools", false, "keep relay tool cache (~/.provasign/tools)")
...
This workspace uses [Relay](https://github.com/provasign/provasign) to
...
3. **Treat ` + "`high`" + ` / ` + "`critical`" + ` findings as blocking.** They block both the
   pre-commit hook and ` + "`provasign cert`" + `. Fix or annotate; do not ` + "`--no-verify`" + `.

4. **Use ` + "`provasign tools status`" + ` to verify analyzer binaries are present** before
...
| Inspect what blocked a commit      | ` + "`provasign cert show`" + `               |
| Verify environment                 | ` + "`provasign doctor`" + `                  |
`
