...
func parseDaemonSocketFlag(args []string) (sock string, err error) {
	...
	sockFlag := fg.String("socket", "", "unix socket path (defaults to ~/.provasign/daemon.sock)")
...
func cmdDaemonStart(args []string, stdout, stderr *os.File) int {
	...
	sockFlag := fs.String("socket", "", "unix socket path (defaults to ~/.provasign/daemon.sock)")
	fs.SetOutput(stderr)
