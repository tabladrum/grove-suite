...
import (
	...
	"github.com/provasign/provasign/internal/sonarqube"
)
...
// at .provasign/rulesets/<name>.xml so the Phase 2B SonarLint Core engine can
...
func cmdImportSonarQube(args []string) int {
	...
	outDir := fs.String("out", ".provasign/rulesets", "output directory for the imported XML")
	...
	if !*quiet {
		...
		if !*withSonar {
			fmt.Println()
			fmt.Println("next: run `provasign tools install --with-sonar` (Phase 2B) to enable")
			fmt.Println("      evaluation of this profile against your code at relay_check time.")
		}
		fmt.Println()
		fmt.Println("Add this to .provasign/provasign.yaml so it's picked up on every invocation:")
		fmt.Println("  sonar:")
