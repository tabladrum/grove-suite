...
// It lives at <intent-store>/.provasign-outbox-cursor and is the source of
// truth for resume — git history is read-only audit.
const CursorFile = ".provasign-outbox-cursor"

// LockFile guards Push against concurrent invocations on the same intent
// store. Lives at <intent-store>/.provasign-outbox.lock.
const LockFile = ".provasign-outbox.lock"
