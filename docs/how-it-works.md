...
layout: default
nav_order: 3
description: "The Relay loop: capture the intent, gate in the agent's loop, certify with Ed25519, replay the evidence forever."
...
Relay moves the quality gate **into the agent's loop** instead of leaving it at the end of the pipeline, and it turns every admitted change into a cryptographically verifiable record.
...
Prompt
  │
  ▼
① Intent captured as YAML  ──────────────►  .relay/intents/INT-….yaml
  │                                          (the verbatim prompt + agent/model identity)
  ▼
② Agent writes code
  │
  ▼
③ relay_check   (in-loop, sub-10s)
  ...
  │  (agent satisfied)
  ▼
④ relay_certify  (full suite at commit time)
...
⑤ Ed25519-signed admission  ─────────────►  linear commit + certificate
  ...
  ▼
⑥ relay cert replay <id>   (any time later)  →  byte_reproducible / tool_drift / config_drift
```
...
Before any code changes, the prompt that produced the code is captured as a YAML intent. In a draft it lives at `.relay/.cache/intents/` (gitignored); when the task closes it is **promoted** to `.relay/intents/` and committed, so it travels with the repo.
...
`relay_check` runs the fast gates *while the agent is still working*: SAST on the changed files plus the unit tests Grove identifies as affected by the change. It returns structured findings — file, line, rule, severity, fix-hint — in under ten seconds, so the agent self-corrects before it ever opens a PR.

## ④ Certify

When the code is ready, `relay_certify` runs the full suite in a git-worktree-isolated build:

- **Stage 1** — build + full test run + coverage of the *changed* symbols (measured against Grove's `tests` edges).
- **Stage 2** — secrets (gitleaks + inline), SAST (semgrep, OWASP + your `.relay/rulesets/`), dependency audit (govulncheck / npm audit / pip-audit), language linters.
...
# Certificate-ID: relay-cert-abc123
...
`relay cert replay <id>` re-runs the gates against the recorded changeset and classifies any divergence:
...
| `config_drift` | The effective `.relay/` config hash differs from the certificate's |
