...
// these via `provasign tools sync` (added in MVP-L4b) which fetches and verifies.
var Registry = map[string][]Release{
	...
	// system pip/pipx install. `provasign tools install semgrep` returns an
	// explanatory error pointing the user at pipx.

	// jre: Eclipse Temurin 21 LTS. Pinned per OS/arch. Required for the
	// SonarLint Core wrapper. Extracted into ~/.provasign/tools/jre/ with a
	// bin/java symlink the sonar analyzer resolves at run time.
	// SHAs left empty (see header note); `provasign tools sync` regenerates.
	"jre": {
		{
