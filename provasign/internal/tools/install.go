...
// DefaultRoot returns ~/.provasign/tools (or PROVASIGN_TOOLS_ROOT when set).
func DefaultRoot() (string, error) {
	if env := os.Getenv("PROVASIGN_TOOLS_ROOT"); env != "" {
		return env, nil
	}
	...
	return filepath.Join(home, ".provasign", "tools"), nil
}
...
// just `provasign tools install --only=sonar-php` with the matching
// registry entry.
