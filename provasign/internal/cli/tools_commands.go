...
// `provasign tools <install|uninstall|list|which>` — manage Stage-2 binaries.
...
import (
	...
	"github.com/provasign/provasign/internal/analyzers/sonar"
...
// RunTools dispatches `provasign tools <sub>`.
func RunTools(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: provasign tools <install|uninstall|list|status|which> [args]")
		return 1
	}
	switch args[0] {
	case "install":
...
func cmdToolsInstall(args []string) int {
	...
	root := fs.String("root", "", "override install root (default: $PROVASIGN_TOOLS_ROOT or ~/.provasign/tools)")
...
func cmdToolsUninstall(args []string) int {
	...
	fmt.Printf("removed provasign tools cache: %s\n", inst.Root)
	return 0
}
...
func cmdToolsStatus(args []string) int {
	...
	startDir := fs.String("dir", ".", "start directory for .provasign/ discovery")
	if err := fs.Parse(args); err != nil {
		return 1
	}
...
func cmdToolsWhich(args []string) int {
	if len(args) != 1 {
		fmt.Fprintln(os.Stderr, "usage: provasign tools which <name>")
		return 1
	}
	name := args[0]
	path := tools.Locate(name)
	if path == "" {
		fmt.Fprintf(os.Stderr, "%s: not found (run `provasign tools install`)\n", name)
		return 1
	}
	fmt.Println(path)
	return 0
...
// printToolsStatus joins the analyzer config from .provasign/provasign.yaml with
...
func printToolsStatus(w io.Writer, startDir string) {
	...
	if err != nil {
		fmt.Fprintf(w, "no .provasign/provasign.yaml found from %s (using defaults)\n\n", startDir)
		cfg = nil
	} else {
...
func sonarBinaryStatus() (string, string) {
	...
	case java != "" && jar != "":
		return "ok", "java=" + java
	case java == "" && jar == "":
		return "missing", "run `provasign tools install --with-sonar`"
	case java == "":
...
func analyzerBinary(name string, cfg core.AnalyzerConfig) (string, string) {
	...
	case "gitleaks":
		return "gitleaks", "run `provasign tools install`"
	case "semgrep":
		return "semgrep", "pipx install semgrep"
	case "govulncheck":
		return "govulncheck", "run `provasign tools install`"
	case "eslint":
