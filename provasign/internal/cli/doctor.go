// provasign doctor — single command that diagnoses the four things most
...
//  3. relay config: is .provasign/provasign.yaml parseable and what scope/floor
...
func RunDoctor(args []string) int {
	...
	startDir := fs.String("dir", ".", "start directory for .provasign/ + .grove/ discovery")
...
func checkConfig(w io.Writer, dir string) int {
	...
	if err != nil {
		fmt.Fprintf(w, "  status:     no .provasign/provasign.yaml from %s (using built-in defaults)\n", dir)
		return 0
	}
...
func checkJVM(w io.Writer) int {
	...
	if java == "" && jar == "" {
		fmt.Fprintln(w, "  hint:       run `provasign tools install --with-sonar`")
	} else if java == "" {
		fmt.Fprintln(w, "  hint:       set JAVA_HOME to a Java 17+ install or run `provasign tools install --with-sonar`")
	} else {
		fmt.Fprintln(w, "  hint:       run `provasign tools install --with-sonar` to fetch the wrapper jar")
	}
