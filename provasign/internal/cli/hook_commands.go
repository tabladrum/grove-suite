...
//
// `provasign hook install [--force]` writes the pre-push hook.
// `provasign hook uninstall` removes the relay-managed hook.
// `provasign outbox push --intent-store=<path> [--batch=N]` replays new
...
import (
	...
	"github.com/provasign/provasign/internal/analyzers/gitleaks"
	"github.com/provasign/provasign/internal/analyzers/semgrep"
	...
	"github.com/provasign/provasign/internal/githook"
	"github.com/provasign/provasign/internal/outbox"
)

// RunHook dispatches `provasign hook <install|uninstall>`.
...
func runHookWith(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "usage: provasign hook <install|install-all|uninstall|pre-commit|run> [--repo <path>] [--force] [--pre-commit]")
		return 1
	}
	sub := args[0]
...
// cmdHookRun is the implementation of `provasign hook run`, called by the
...
// blocking gate (use `provasign cert` for hard blocking).
func cmdHookRun(repoRoot string, stderr io.Writer) int {
	...
	dbPath := filepath.Join(repoRoot, ".provasign", "relay.db")
	...
	fmt.Fprintf(os.Stderr, "provasign: %d certified change-set(s) in local outbox — run `provasign outbox push` to sync\n", len(certs))
	return 0
}
...
// RunOutbox dispatches `provasign outbox <push>`.
...
func runOutboxWith(args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 || args[0] != "push" {
		fmt.Fprintln(stderr, "usage: provasign outbox push --intent-store <path> [--repo <path>] [--batch N]")
		return 1
	}
	fs := flag.NewFlagSet("outbox push", flag.ContinueOnError)
	repo := fs.String("repo", ".", "path to relay-managed repo (locates .provasign/relay.db)")
	...
	dbPath := filepath.Join(repoAbs, ".provasign", "engine.db")
	store, err := enginestore.Open(dbPath)
	if err != nil {
