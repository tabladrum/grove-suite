...
// TestKeysGenCreatesKey covers the laptop-bootstrap path: a fresh `.relay/`
...
func TestKeysGenCreatesKey(t *testing.T) {
	...
	priv := filepath.Join(dir, ".relay", "keys", "signing.ed25519.key")
...
func TestKeysExportImportRoundTrip(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir2, ".relay", "keys", "signing.ed25519.key")); err != nil {
		t.Errorf("imported priv key missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir2, ".relay", "keys", "signing.ed25519.pub")); err != nil {
...
func TestKeysImportRefusesUnexpectedFiles(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dest, ".relay", "keys", "etc")); err == nil {
