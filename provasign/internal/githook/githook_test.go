...
func TestInstall_FreshRepo(t *testing.T) {
	...
	if !strings.Contains(string(body), "provasign hook run") {
		t.Error("body missing provasign hook run invocation")
	}
...
func TestInstallPreCommit_FreshRepo(t *testing.T) {
	...
	if !strings.Contains(string(body), "provasign hook pre-commit") {
		t.Error("pre-commit body missing 'provasign hook pre-commit' invocation")
	}
	if runtime.GOOS != "windows" {
		info, _ := os.Stat(path)
		if info.Mode().Perm()&0o100 == 0 {
