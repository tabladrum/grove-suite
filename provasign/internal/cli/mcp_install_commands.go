...
// `provasign mcp install-for <client>` — register the laptop Relay daemon's
...
func cmdMCPInstallFor(args []string) int {
	...
	repo := fs.String("repo", "", "default repo for provasign mcp serve (passed via args)")
	bin := fs.String("bin", "", "absolute path to the provasign binary (defaults to the running executable)")
	...
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: provasign mcp install-for <client> [--uninstall] [--repo <dir>] [--bin <path>]")
		fmt.Fprintln(os.Stderr, "       provasign mcp install-for --list")
		return 1
	}
	clientID := fs.Arg(0)
...
// on repeated provasign init runs, which resets Claude Code's MCP approval state.
...
// interactive re-approval after every `provasign init` run.
