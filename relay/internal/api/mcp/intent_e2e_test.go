...
func TestIntentCaptureRoundTrip(t *testing.T) {
	repo := t.TempDir()
	if err := os.MkdirAll(filepath.Join(repo, ".relay"), 0o755); err != nil {
		t.Fatal(err)
	}
	s := NewServer(repo, buildFakeEngine(&fakeGate{name: "x", verdict: core.VerdictAllow}))

	openResult := callTool(t, s, "relay_intent_open", map[string]any{
	...
	upd := callTool(t, s, "relay_intent_update", map[string]any{
		"intent_id": intentID,
	...
	// Close: promotion.
	close := callTool(t, s, "relay_intent_close", map[string]any{
		"intent_id": intentID,
		"agent":     "claude-code:1.4.2",
	})
	...
	// List.
	listResult := callTool(t, s, "relay_intent_list", map[string]any{"status": "committed"})
